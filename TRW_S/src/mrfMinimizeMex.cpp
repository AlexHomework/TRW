...
void withLabelsMatrix(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	// Version of the function below which assumes full labels matrix.
	// FIXME(bihaqo@gmail.com): it should be implemented accurately, without nasty copy-paste
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if(mInPtr != NULL){
		if(!mxIsEmpty(mInPtr)){
			// If user specified full labels matrix
			withLabelsMatrix(nlhs, plhs, nrhs, prhs);
			return;
		}
	}
	...
	std::vector<TypePotts::REAL> energy, lowerBound;
	std::vector<clock_t> time_arr;
	
	TypePotts::REAL *D = new TypePotts::REAL[numLabels];
	TypePotts::REAL *P = new TypePotts::REAL[numLabels * numLabels];
	for(int i = 0; i < numLabels * numLabels; ++i)
			P[i] = 0;
